...
namespace FriendOrganizer.UI.ViewModel
{
    public abstract class DetailViewModelBase : ViewModelBase, IDetailViewModel
    {
        ...
        protected async virtual void OnCloseDetailViewExecute()
        {
            if (HasChanges)
            {
                var result = await MessageDialogService.ShowOkCancelDialogAsync("You've made changes. " +
                    "Close this item?", "Question");
        ...
        protected async Task SaveWithOptimisticConcurrencyAsync(Func<Task> saveFunc, Action afterSaveAction)
        {
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                var result = await MessageDialogService.ShowOkCancelDialogAsync("The enetity has been changed in" +
                    "the meantime by someone else. Click OK to save your changes. Click Cancel to reload" +
                    "the entity from database.", "Question");
